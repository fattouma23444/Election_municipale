...
void
on_btncrtr_clicked                     (GtkButton       *button,
                                        gpointer         user_data)
{
...
switch (chckcrtr)
{
	case 1:
		...
		if (w==0)
			gtk_label_set_text(GTK_LABEL(otpt1),strcat(strcpy(k,gtk_entry_get_text(GTK_ENTRY(ipt1)))," introuvable "));
		else
			gtk_label_set_text(GTK_LABEL(otpt1),strcat(strcpy(k,gtk_entry_get_text(GTK_ENTRY(ipt1)))," trouvable "));
	break;
	case 2:
		ipt2 = lookup_widget(objet_graphique, "crtr2entry") ;
		otpt1 = lookup_widget(objet_graphique,"labelcrtr");
		gtk_label_set_text(GTK_LABEL(otpt1),gtk_entry_get_text(GTK_ENTRY(ipt2)));
...
void
on_annulercrtr_clicked                 (GtkButton       *button,
                                        gpointer         user_data)
{
