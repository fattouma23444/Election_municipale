...
if (g.N_CIN == -1 )
	{
...
else
		{
			...
			if (g.Gender == 'M')
				gtk_toggle_button_set_active(GTK_TOGGLE_BUTTON(en8),TRUE);
			else
				gtk_toggle_button_set_active(GTK_TOGGLE_BUTTON(en9),TRUE);
...
void
on_menuprincipalebtn4_clicked          (GtkWidget       *button,
                                        gpointer         user_data)
{
GtkWidget *GDU7 = lookup_widget(button,"GDU7");
gtk_widget_destroy(GDU7);
GtkWidget *dashstats = create_dashstats();
set_event_box_background(dashstats,"/home/lou/Projects/project2/src/Bkgrnd.jpg");
gtk_widget_show(dashstats);
}
...
void
on_stat1_clicked                       (GtkWidget       *button,
                                        gpointer         user_data)
{
GdkColor clrs;
GtkWidget *ms1 = lookup_widget(button,"stat1msg");
int nbrev=0,elecv=0;
TPE ( "GDU.txt", &nbrev , &elecv);
char affich1ch[20];
sprintf(affich1ch,"%.2f",(float)nbrev/elecv * 100);
strcat(affich1ch," %");
gdk_color_parse ("blue", &clrs);
gtk_widget_modify_fg (ms1, GTK_STATE_NORMAL, &clrs);
gtk_label_set_text(GTK_LABEL(ms1),affich1ch);
}


void
on_stat2_clicked                       (GtkWidget       *button,
                                        gpointer         user_data)
{
GdkColor clrs;
int femme=0,homme=0;
char str[50];
int z = TPHF( "GDU.txt", &femme, &homme);
sprintf(str, "Femmes : %.2f %%  Hommes : %.2f %% \n",(float)femme/z *100,(float)homme/z *100);
GtkWidget *ms2 = lookup_widget(button,"stat2msg");
gdk_color_parse ("blue", &clrs);
gtk_widget_modify_fg (ms2, GTK_STATE_NORMAL, &clrs);
gtk_label_set_text(GTK_LABEL(ms2),str);
}


void
on_stat3_clicked                       (GtkWidget       *button,
                                        gpointer         user_data)
{
GtkWidget *ms3 = lookup_widget(button,"stat3msg");
GdkColor clrs;
int nbrfa = 0, nbvf = 0;
TVB("GDU.txt",&nbrfa,&nbvf);
char affich2ch[20];
sprintf(affich2ch,"%.2f",(float)nbrfa/nbvf * 100);
strcat(affich2ch," %");
gdk_color_parse ("blue", &clrs);
gtk_widget_modify_fg (ms3, GTK_STATE_NORMAL, &clrs);
gtk_label_set_text(GTK_LABEL(ms3),affich2ch);
}


void
on_stat4_clicked                       (GtkWidget       *button,
                                        gpointer         user_data)
{
GtkWidget *ms4 = lookup_widget(button,"stat4msg");
GdkColor clrs;
float j=agemoyen("GDU.txt");
char affich3ch[30];
sprintf(affich3ch,"Age Moyen : %.1f ans",j);
gdk_color_parse ("blue", &clrs);
gtk_widget_modify_fg (ms4, GTK_STATE_NORMAL, &clrs);
gtk_label_set_text(GTK_LABEL(ms4),affich3ch);
}

GtkWidget *cfrstat5;

void
on_stat5_clicked                       (GtkWidget       *button,
                                        gpointer         user_data)
{
GtkWidget *dashstats = lookup_widget(button,"dashstats");
cfrstat5 = dashstats;
GtkWidget *entrybvstat = create_entrybvstat();
set_event_box_background(entrybvstat,"/home/lou/Projects/project2/src/Bkgrndbv.jpg");
gtk_widget_show(entrybvstat);
}


void
on_stat6_clicked                       (GtkWidget       *button,
                                        gpointer         user_data)
{
reclamation r;
int totalrecs = 0;
...
GtkWidget *ms6 = lookup_widget(button,"stat6msg");
GdkColor clrs;
char affich5ch[50];
sprintf(affich5ch,"Total Réclamations : %d",totalrecs);
gdk_color_parse ("blue", &clrs);
gtk_widget_modify_fg (ms6, GTK_STATE_NORMAL, &clrs);
gtk_label_set_text(GTK_LABEL(ms6),affich5ch);
}


void
on_btnrtrstat_clicked                  (GtkWidget       *button,
                                        gpointer         user_data)
{
GdkColor clrs;
GtkWidget *ebvs = lookup_widget(button,"dashstats");
gtk_widget_destroy(ebvs);
GtkWidget *GDU7 = create_GDU7();
set_event_box_background(GDU7,"/home/lou/Projects/project2/src/Bkgrnd2.jpg");
GtkWidget *labelmf = lookup_widget(GDU7,"lblvecid");
gdk_color_parse ("red", &clrs);
...
void
on_cfrbv_clicked                       (GtkWidget       *button,
                                        gpointer         user_data)
{
GtkWidget *entrybv = lookup_widget(button,"entrybvn");
int x = NBE("GDU.txt",atoi(gtk_entry_get_text(GTK_ENTRY(entrybv))));
GtkWidget *entrybsw = lookup_widget(button,"entrybvstat");
GtkWidget *ms5 = lookup_widget(cfrstat5,"stat5msg");
GdkColor clrs;
char affich4ch[50];
sprintf(affich4ch,"Bureau : %d Electeurs : %d ",atoi(gtk_entry_get_text(GTK_ENTRY(entrybv))),x);
gdk_color_parse ("blue", &clrs);
gtk_widget_modify_fg (ms5, GTK_STATE_NORMAL, &clrs);
gtk_label_set_text(GTK_LABEL(ms5),affich4ch);
gtk_widget_destroy(entrybsw);
}
