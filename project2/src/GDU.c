...
void TVB( char * nom_fichier,int *nbr_VB,int *nbr_V)
{
    GDU s;
    FILE *f;
...
if ( f!=NULL )
    {
	    ...
	    if (strcmp(s.TypeUtilisateur,"Electeur") == 0 && s.vote != -1)
			(*nbr_V)++;
	    if (strcmp(s.TypeUtilisateur,"Electeur") == 0 && s.vote == 0)
			(*nbr_VB)++;
        }
    }
    fclose(f);
...
void set_event_box_background(GtkWidget* event_box, char path[])
{
    GError* error = NULL;
